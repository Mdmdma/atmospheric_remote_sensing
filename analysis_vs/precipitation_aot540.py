...
plt.scatter(calitoo_422['Date'], calitoo_422['AOT540'])
...
plt.savefig("aot_cal_filtered_2.png", dpi=300, bbox_inches="tight")
...
# plot filtered timeseries of all data
plt.figure(figsize=(10, 6))
plt.scatter(calitoo_all['day_of_year'], calitoo_all['AOT540'])
...
ax1.scatter(p_cal['day_of_year'],p_cal['AOT540'],color='red',marker='x')
...
ax1.scatter(p_cal['day_of_year'], p_cal['AOT540'], color='red', marker='x', label='AOT540')
ax1.set_ylabel('AOT540', color='red')
...
# Show the plot
plt.tight_layout()
plt.savefig("P_aot_540.png", dpi=300, bbox_inches="tight")
plt.show()

##
## correlation
p_cal_clean = p_cal.dropna(subset=['AOT540'])
p_cal_clean = p_cal_clean[p_cal_clean['AOT540'] != 0]

# cor coefficient
from scipy.stats import spearmanr

cor_coeff, p_value = spearmanr(p_cal_clean['p'],p_cal_clean['AOT540'])

cor_coeff = round(cor_coeff,3)
p_value = f"{p_value:.2e}"

plt.figure(figsize=(8, 7))
scatter = plt.scatter(p_cal_clean['p'],p_cal_clean['AOT540'],marker='x')
...
plt.grid(True)
plt.savefig("cor_p_540.png", dpi=300, bbox_inches="tight")
plt.show()


## precipitation shift one day

cor_coeff_prev, p_value_prev = spearmanr(p_cal_clean['prev_day_p'],p_cal_clean['AOT540'])

cor_coeff_prev = round(cor_coeff_prev,3)
p_value_prev = f"{p_value_prev:.2e}"

plt.figure(figsize=(8, 7))
scatter = plt.scatter(p_cal_clean['prev_day_p'],p_cal_clean['AOT540'],marker='x')
...
plt.grid(True)
plt.savefig("cor_p_prev_540.png", dpi=300, bbox_inches="tight")
plt.show()
